...
def idp(monkeypatch, rsa_anahtar):
    ...
    def id_token_uret(*, nonce: str, eposta: str = "sso.kullanici@ornek.local", sub: str = "sub-1") -> str:
        simdi = int(time.time())
        return jwt.encode(
            {
                "iss": ISSUER,
...
async def _saglayici_ekle(org_id: int, *, slug: str = "test-idp", etkin: bool = True) -> SsoSaglayici:
...
def test_kullanici_bilgisi_claim_eslemesi():
    ...
    assert bilgi == {"dis_id": "s1", "eposta": "ali@ornek.local", "ad": "Ali Veli"}


def test_state_tek_kullanimlik_ve_sureli(monkeypatch):
