...
/**
 * React dışı yardımcılar (ör. `lib/api.ts`, `lib/dogrulama.ts`) için aktif dil:
 * önce `localStorage`, sonra çerez, yoksa varsayılan. Tarayıcı dışında
 * (sunucu/SSR) varsayılan dil döner.
 */
export function aktifDil(): Dil {
  if (typeof document === "undefined") return VARSAYILAN_DIL;
  try {
    const kayitli = window.localStorage.getItem(DIL_CEREZI);
    if (dilGecerliMi(kayitli)) return kayitli;
  } catch {
    // Depo kapalı; çereze düşülür.
  }
  const satir = document.cookie
    .split("; ")
    .find((parca) => parca.startsWith(`${DIL_CEREZI}=`));
  const deger = satir?.slice(DIL_CEREZI.length + 1);
