...
/** Dil seçici; onuc'ta `/hesap`, panelde üst barda kullanılır (spec §10.2). */
export function DilSecici({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <label htmlFor={seciciId} className="text-[13px] text-neutral-500">
        {t("genel.dil")}
      </label>
      <select
        id={seciciId}
        ...
        className={cn(
          "h-8 rounded-md border border-neutral-200 bg-white px-2 text-[13px] text-neutral-900 transition-colors",
